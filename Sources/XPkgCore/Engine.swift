...
public class Engine: CommandEngine {
  static let missingProductPattern = try! NSRegularExpression(
    pattern: #"'vault': product '(.*)' required by package 'vault' target 'Installed' not found in package '(.*)'."#)
  ...
  internal var websitesURL: URL {
    let paths = ["~/Developer/Websites"]
    for path in paths {
      let url = URL(fileURLWithPath: (path as NSString).expandingTildeInPath)
      if FileManager.default.fileExists(at: url) {
        return url
    ...
    return URL(fileURLWithPath: "Websites")
  }
  ...
  func attempt(action: String, cleanup: (() throws -> Void)? = nil, block: () throws -> Void) {
    verbose.log(action)
    do {
      try block()
    }
  ...
  func forEachPackage(_ block: (Package) -> Void) throws -> Bool {
